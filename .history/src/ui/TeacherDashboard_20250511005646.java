    ...
    private JPanel createQuizzesPanel() {
        ...
        editQuizButton.addActionListener(e -> {
            ...
            if (selectedIndex >= 0) {
                ...
                System.out.println("Selected Quiz: " + selected); // Debugging statement

                // Split the selected string to extract quiz title and due date
                String[] parts = selected.split(" - Due: ");
                if (parts.length > 1) { // Ensure the array has at least 2 elements
                    String currentQuizTitle = parts[0];
                    LocalDate currentDueDate;
                    try {
                        currentDueDate = LocalDate.parse(parts[1]);
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(this,
                                "Invalid date format in quiz data.",
                                "Error", JOptionPane.ERROR_MESSAGE);
                        return;
                    }

                    // Create input panel for editing quiz
