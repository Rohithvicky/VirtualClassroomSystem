...
public class StudentDashboard extends JFrame {
    ...
    private void takeQuiz(String quizTitle) {
        // Fetch the quiz details from the database or file
        Quiz quiz = DataManager.getInstance().getQuiz(quizTitle);

        if (quiz != null) {
            JPanel quizPanel = new JPanel(new GridLayout(0, 1, 5, 5));
            JLabel questionLabel = new JLabel("<html>" + quiz.getQuestion() + "</html>");
            JRadioButton optionA = new JRadioButton(quiz.getOptionA());
            JRadioButton optionB = new JRadioButton(quiz.getOptionB());
            JRadioButton optionC = new JRadioButton(quiz.getOptionC());
            JRadioButton optionD = new JRadioButton(quiz.getOptionD());
            ...
            int result = JOptionPane.showConfirmDialog(this, quizPanel, "Take Quiz", JOptionPane.OK_CANCEL_OPTION);
            if (result == JOptionPane.OK_OPTION) {
                ...
                if (selectedAnswer != null) {
                    boolean isCorrect = selectedAnswer.equals(quiz.getCorrectAnswer());
                    JOptionPane.showMessageDialog(this, isCorrect ? "Correct Answer!" : "Wrong Answer!", "Result", JOptionPane.INFORMATION_MESSAGE);
                } else {
