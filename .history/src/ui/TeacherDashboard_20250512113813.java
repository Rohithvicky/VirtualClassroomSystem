...
public class TeacherDashboard extends JFrame {
    ...
    // New list models for submissions
    private DefaultListModel<String> quizSubmissionListModel;
    private DefaultListModel<String> assignmentSubmissionListModel;
    ...
    private void initializeComponents() {
        ...
        // New panel for Submissions (quiz & assignment submissions)
        JPanel submissionsPanel = createSubmissionsPanel();
        ...
        ImageIcon submissionsIcon = createIcon("submissions_icon.png", "Submissions");
        ...
        tabbedPane.addTab("Submissions", submissionsIcon, submissionsPanel);
    ...
    private JPanel createClassesPanel() {
        ...
        // Button panel for class actions (add, edit, delete)
    ...
    private JPanel createQuizzesPanel() {
        ...
        // Button panel for quiz actions (add quiz, add question, view questions, delete
        // quiz)
        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 0));
        buttonPanel.setBackground(backgroundColor);
        ...
        viewQuestionsButton.addActionListener(e -> {
            ...
            if (selectedIndex >= 0) {
                ...
                if (!questionsArray.isEmpty()) {
                    ...
                    JTextArea questionArea = new JTextArea(sb.toString());
    ...
    private JPanel createAssignmentsPanel() {
        ...
        // Button panel for assignment actions (add & delete)
        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 0));
        buttonPanel.setBackground(backgroundColor);
    ...
    // New panel for Submissions
    private JPanel createSubmissionsPanel() {
        // Create a sub-tab pane for quiz and assignment submissions
        JTabbedPane submissionsTabPane = new JTabbedPane();
        submissionsTabPane.setFont(titleFont);
        submissionsTabPane.setBackground(backgroundColor);

        // Initialize list models for submissions
        quizSubmissionListModel = new DefaultListModel<>();
        assignmentSubmissionListModel = new DefaultListModel<>();

        // Panel for quiz submissions
        JPanel quizSubmissionsPanel = new JPanel(new BorderLayout(10, 10));
        quizSubmissionsPanel.setBackground(backgroundColor);
        // List for quiz submissions
        JList<String> quizSubmissionsList = new JList<>(quizSubmissionListModel);
        quizSubmissionsList.setFont(regularFont);
        quizSubmissionsList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        quizSubmissionsList.setFixedCellHeight(40);
        quizSubmissionsList.setCellRenderer(new CustomListCellRenderer());
        JScrollPane quizSubmissionsScroll = new JScrollPane(quizSubmissionsList);
        quizSubmissionsScroll.setBorder(BorderFactory.createLineBorder(accentColor, 1));
        quizSubmissionsPanel.add(quizSubmissionsScroll, BorderLayout.CENTER);
        // Refresh button for quiz submissions
        JPanel quizSubmissionsButtonPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 0));
        quizSubmissionsButtonPanel.setBackground(backgroundColor);
        JButton refreshQuizSubmissionsButton = createStyledButton("Refresh Quiz Submissions", accentColor);
        refreshQuizSubmissionsButton.addActionListener(e -> {
            loadQuizSubmissions();
        });
        quizSubmissionsButtonPanel.add(refreshQuizSubmissionsButton);
        quizSubmissionsPanel.add(quizSubmissionsButtonPanel, BorderLayout.SOUTH);

        // Panel for assignment submissions
        JPanel assignmentSubmissionsPanel = new JPanel(new BorderLayout(10, 10));
        assignmentSubmissionsPanel.setBackground(backgroundColor);
        // List for assignment submissions
        JList<String> assignmentSubmissionsList = new JList<>(assignmentSubmissionListModel);
        assignmentSubmissionsList.setFont(regularFont);
        assignmentSubmissionsList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        assignmentSubmissionsList.setFixedCellHeight(40);
        assignmentSubmissionsList.setCellRenderer(new CustomListCellRenderer());
        JScrollPane assignmentSubmissionsScroll = new JScrollPane(assignmentSubmissionsList);
        assignmentSubmissionsScroll.setBorder(BorderFactory.createLineBorder(accentColor, 1));
        assignmentSubmissionsPanel.add(assignmentSubmissionsScroll, BorderLayout.CENTER);
        // Refresh button for assignment submissions
        JPanel assignmentSubmissionsButtonPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 0));
        assignmentSubmissionsButtonPanel.setBackground(backgroundColor);
        JButton refreshAssignmentSubmissionsButton = createStyledButton("Refresh Assignment Submissions", accentColor);
        refreshAssignmentSubmissionsButton.addActionListener(e -> {
            loadAssignmentSubmissions();
        });
        assignmentSubmissionsButtonPanel.add(refreshAssignmentSubmissionsButton);
        assignmentSubmissionsPanel.add(assignmentSubmissionsButtonPanel, BorderLayout.SOUTH);

        // Add both panels as tabs in the submissions tab pane
        submissionsTabPane.addTab("Quiz Submissions", quizSubmissionsPanel);
        submissionsTabPane.addTab("Assignment Submissions", assignmentSubmissionsPanel);

        // Wrap in a parent panel to return
        JPanel parentPanel = new JPanel(new BorderLayout());
        parentPanel.add(submissionsTabPane, BorderLayout.CENTER);
        return parentPanel;
    }
    ...
    private void loadData() {
        ...
        // Also load submissions data for teacher dashboard
        loadQuizSubmissions();
        loadAssignmentSubmissions();
    }
    ...
    // New method to load quiz submissions from DataManager
    private void loadQuizSubmissions() {
        quizSubmissionListModel.clear();
        // Assuming DataManager.getQuizSubmissions() returns List<String>
        List<String> submissions = DataManager.getInstance().getQuizSubmissions();
        submissions.forEach(quizSubmissionListModel::addElement);
    }

    // New method to load assignment submissions from DataManager
    private void loadAssignmentSubmissions() {
        assignmentSubmissionListModel.clear();
        // Assuming DataManager.getAssignmentSubmissions() returns List<String>
        List<String> submissions = DataManager.getInstance().getAssignmentSubmissions();
        submissions.forEach(assignmentSubmissionListModel::addElement);
    }
