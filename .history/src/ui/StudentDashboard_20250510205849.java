...
public class StudentDashboard extends JFrame {
    ...
    private void takeQuiz(String quizTitle) {
        System.out.println("Quiz Title Selected: " + quizTitle); // Debug statement
        Quiz quiz = DataManager.getInstance().getQuiz(quizTitle);
        if (quiz != null) {
            System.out.println("Quiz Found: " + quiz.getTitle()); // Debug statement
            int result = JOptionPane.showConfirmDialog(this,
                    "Do you want to download the quiz document?",
                    "Download Quiz", JOptionPane.YES_NO_OPTION);

            if (result == JOptionPane.YES_OPTION) {
                JFileChooser fileChooser = new JFileChooser();
                fileChooser.setSelectedFile(new File(quiz.getDocumentName()));
                int saveResult = fileChooser.showSaveDialog(this);
                if (saveResult == JFileChooser.APPROVE_OPTION) {
                    // Simulate downloading the file
                    File destination = fileChooser.getSelectedFile();
                    try {
                        Files.copy(Paths.get("path/to/uploaded/documents/" + quiz.getDocumentName()),
                                destination.toPath(),
                                StandardCopyOption.REPLACE_EXISTING);
                        JOptionPane.showMessageDialog(this,
                                "Quiz document downloaded successfully!",
                                "Success", JOptionPane.INFORMATION_MESSAGE);
                    } catch (IOException e) {
                        JOptionPane.showMessageDialog(this,
                                "Failed to download quiz document: " + e.getMessage(),
                                "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
        } else {
            System.out.println("Quiz not found in database."); // Debug statement
