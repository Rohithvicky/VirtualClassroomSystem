...
public class TeacherDashboard extends JFrame {
    ...
    private JPanel createQuizzesPanel() {
        ...
        JButton editQuizButton = createStyledButton("Edit Quiz", accentColor);
        ...
        addQuizButton.addActionListener(e -> {
            ...
            if (result == JOptionPane.OK_OPTION && !quizTitle.getText().trim().isEmpty()
                ...
                if (success) {
                    // Ensure the correct format is added to quizListModel
        ...
        editQuizButton.addActionListener(e -> {
            ...
            if (selectedIndex >= 0) {
                ...
                // More robust parsing of the quiz string
                String currentQuizTitle;
                LocalDate currentDueDate;

                try {
                    int separatorIndex = selected.lastIndexOf(" - Due: ");
                    if (separatorIndex == -1) {
                        throw new IllegalArgumentException("Invalid quiz format");
                    }

                    currentQuizTitle = selected.substring(0, separatorIndex);
                    currentDueDate = LocalDate.parse(selected.substring(separatorIndex + 8));
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(this,
                            "Invalid quiz format. Please check the data.\n" + ex.getMessage(),
                            "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                // Rest of the edit dialog code remains the same...
                JPanel inputPanel = new JPanel(new GridLayout(3, 2, 5, 10));
                JTextField quizTitle = new JTextField(currentQuizTitle);
                JComboBox<String> classSelector = new JComboBox<>();
                // Calculate days remaining
                long daysRemaining = ChronoUnit.DAYS.between(LocalDate.now(), currentDueDate);
                int clampedDaysRemaining = (int) Math.max(1, Math.min(daysRemaining, 30)); // Clamp to range [1, 30]

                JSpinner dueDateSpinner = new JSpinner(new SpinnerNumberModel(
                        clampedDaysRemaining, 1, 30, 1));
                ...
                int result = JOptionPane.showConfirmDialog(this, inputPanel,
                        "Edit Quiz", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

                if (result == JOptionPane.OK_OPTION && !quizTitle.getText().trim().isEmpty()
                        && classSelector.getSelectedIndex() > 0) {

                    LocalDate newDueDate = LocalDate.now().plusDays((int) dueDateSpinner.getValue());

                    // Update quiz in the database
                    boolean success = DataManager.getInstance().updateQuiz(
                            currentQuizTitle, // Old quiz title
                            quizTitle.getText().trim(), // New quiz title
                            1, // Replace with actual class ID
                            newDueDate);

                    if (success) {
                        quizListModel.set(selectedIndex, quizTitle.getText() + " - Due: " + newDueDate);
                        JOptionPane.showMessageDialog(this,
                                "Quiz updated successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(this,
                                "Failed to update quiz.", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
            } else {
                JOptionPane.showMessageDialog(this,
    ...
    private JPanel createAssignmentsPanel() {
        ...
        JButton editAssignmentButton = createStyledButton("Edit Assignment", accentColor);
        ...
        addAssignmentButton.addActionListener(e -> {
            ...
            // Populate class selector (this would normally come from the database)
            classSelector.addItem("Select a class");
            for (int i = 0; i < classListModel.getSize(); i++) {
                String className = classListModel.getElementAt(i).split(" - ")[0];
                classSelector.addItem(className);
            ...
            if (result == JOptionPane.OK_OPTION && !assignmentTitle.getText().trim().isEmpty()
                ...
                // For demo, we use classId = 1. In production, get real class ID
        ...
        editAssignmentButton.addActionListener(e -> {
            ...
            if (selectedIndex >= 0) {
                ...
                JOptionPane.showMessageDialog(this,
                        "Edit functionality to be implemented for: " + selected,
    ...
    private void loadData() {
        ...
        dm.getQuizzes().forEach(quiz -> {
            // Ensure the quiz is in the correct format
            if (!quiz.contains(" - Due: ")) {
                // If not, add the current date as default
