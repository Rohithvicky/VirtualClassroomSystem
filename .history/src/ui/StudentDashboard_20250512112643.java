...
public class StudentDashboard extends JFrame {
    ...
    private DefaultListModel<Quiz> quizListModel; // Changed to hold Quiz objects
    ...
    // Update loadData to update list models and dashboard counts.
    private void loadData() {
        ...
        // Load quizzes (store Quiz objects in the list model)
        dm.getQuizzesObj().forEach(quizListModel::addElement);
        // Load assignments
    ...
    // Changed takeQuiz to work with a Quiz object
    private void takeQuiz(Quiz quiz) {
        List<String[]> questions = DataManager.getInstance().getQuizQuestions(quiz.getTitle());
    ...
    // Renderer for quiz list; displays the quiz title via toString()
    private class QuizListRenderer extends DefaultListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList<?> list, Object value, int index,
            ...
            if (value instanceof Quiz) {
                displayText = ((Quiz) value).getTitle();
            } else {
                displayText = value.toString();
            }
            JLabel label = (JLabel) super.getListCellRendererComponent(list, displayText, index, isSelected,
                    cellHasFocus);
            label.setBorder(BorderFactory.createEmptyBorder(5, 10, 5, 10));
            if (isSelected) {
                label.setBackground(accentColor);
