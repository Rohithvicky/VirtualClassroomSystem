    ...
    private JPanel createQuizzesPanel() {
        ...
        addQuizButton.addActionListener(e -> {
            ...
            if (result == JOptionPane.OK_OPTION && !quizTitleField.getText().trim().isEmpty()) {
                ...
                // Call DataManager.addQuiz to insert a new quiz record. Ensure this method
                // provides a value for the "title" field.
                boolean success = DataManager.getInstance().addQuiz(quizTitleField.getText().trim(), dueDate);
                if (success) {
        ...
        addQuestionButton.addActionListener(e -> {
            // Adding a question requires a quiz to be selected (because a quiz question
            // record must have a title)
            ...
            if (selectedIndex >= 0) {
                ...
                int result = JOptionPane.showConfirmDialog(this, inputPanel,
                        "Add New Question to: " + selectedQuiz, JOptionPane.OK_CANCEL_OPTION,
                ...
                if (result == JOptionPane.OK_OPTION && !questionField.getText().trim().isEmpty()) {
                    ...
                    // Make sure DataManager.addQuizQuestion() uses the question value as the title
                    // so that the 'title' field is not null.
                    boolean success = DataManager.getInstance().addQuizQuestion(
                            selectedQuiz, // Quiz identifier to associate question with a quiz
