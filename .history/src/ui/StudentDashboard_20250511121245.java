...
public class StudentDashboard extends JFrame {
    ...
    private JPanel createStatusBar() {
        ...
        JLabel statusLabel = new JLabel("Connected to Virtual Classroom • " +
...
dm.getQuizzes().forEach(quiz -> {
    quizListModel.addElement(quiz.getTitle()); // Ensure quiz is of type Quiz
});
// Load assignments
dm.getAssignments().forEach(assignment -> {
    assignmentListModel.addElement(assignment.getTitle()); // Ensure assignment is of type Assignment
});
// In the method where you filter assignments
private int getAssignmentIdFromTitle(String title) {
    return DataManager.getInstance().getAssignments().stream()
            .filter(a -> a.getTitle().equals(title)) // Ensure a is of type Assignment
            .findFirst()
            .map(a -> a.getId()) // Ensure a is of type Assignment
            .orElse(-1);
}
    ...
    private void updateDashboardStats() {
        // This would update any dashboard components when data changes
        // Currently handled by recreating the panels when data loads
    }
