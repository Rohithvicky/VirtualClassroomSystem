...
public class StudentDashboard extends JFrame {
    ...
    private JPanel createDashboardPanel() {
        ...
        statsPanel.setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15)); // Reduced padding
    ...
    private JPanel createSmallRoundedStatCard(String title, String value, Color color) {
        JPanel card = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                // Draw rounded rectangle background
                g2.setColor(color);
                g2.fillRoundRect(0, 0, getWidth(), getHeight(), 10, 10); // Smaller corner radius
        ...
        card.setLayout(new BoxLayout(card, BoxLayout.Y_AXIS));
        card.setOpaque(false);
        card.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5)); // Reduced padding

        JLabel titleLabel = new JLabel(title);
        titleLabel.setFont(new Font("Segoe UI", Font.BOLD, 10)); // Smaller font size
        titleLabel.setForeground(Color.WHITE);
        titleLabel.setAlignmentX(Component.CENTER_ALIGNMENT);

        JLabel valueLabel = new JLabel(value);
        valueLabel.setFont(new Font("Segoe UI", Font.BOLD, 18)); // Smaller font size for value
        valueLabel.setForeground(Color.WHITE);
        valueLabel.setAlignmentX(Component.CENTER_ALIGNMENT);

        card.add(titleLabel);
        card.add(Box.createVerticalStrut(3)); // Reduced spacing
        card.add(valueLabel);
