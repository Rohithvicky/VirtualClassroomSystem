...
public class StudentDashboard extends JFrame {
    ...
    private void loadData() {
        ...
        dm.getQuizzes().forEach(quizListModel::addElement); // Ensure quiz titles are loaded
    ...
    private void takeQuiz(String quizTitle) {
        ...
        if (quiz != null) {
            List<String[]> questions = DataManager.getInstance().getQuizQuestions(quizTitle); // Fetch questions
            ...
            for (int i = 0; i < questions.size(); i++) {
                String[] questionData = questions.get(i); // [question, optionA, optionB, optionC, optionD, correctOption]
                JPanel questionPanel = new JPanel(new GridLayout(5, 1));
                questionPanel.setBorder(BorderFactory.createTitledBorder("Question " + (i + 1)));

                JLabel questionLabel = new JLabel(questionData[0]); // Question text
                questionPanel.add(questionLabel);

                buttonGroups[i] = new ButtonGroup();
                for (int j = 0; j < 4; j++) {
                    optionButtons[i][j] = new JRadioButton(questionData[j + 1]); // Options A, B, C, D
            ...
            int result = JOptionPane.showConfirmDialog(this, new JScrollPane(quizPanel),
                    "Take Quiz: " + quizTitle, JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

            if (result == JOptionPane.OK_OPTION) {
                int score = 0;
                for (int i = 0; i < questions.size(); i++) {
                    String correctOption = questions.get(i)[5]; // Correct option
                    for (int j = 0; j < 4; j++) {
                        if (optionButtons[i][j].isSelected() && correctOption.equalsIgnoreCase(String.valueOf((char) ('A' + j)))) {
                            score++;
                        }
                    }
                }

                JOptionPane.showMessageDialog(this, "You scored " + score + " out of " + questions.size() + "!",
