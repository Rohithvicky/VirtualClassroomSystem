...
public class DataManager {
    ...
    public boolean addQuizQuestion(String question, String optionA, String optionB, String optionC, String optionD, String correctOption) {
        Connection conn = null;
        PreparedStatement stmt = null;

        try {
            conn = DBConnection.getConnection();
            String sql = "INSERT INTO quizzes (question, option_a, option_b, option_c, option_d, correct_option) VALUES (?, ?, ?, ?, ?, ?)";
            stmt = conn.prepareStatement(sql);
    ...
    public List<String> getQuizQuestions(String quizTitle) {
        ...
        try {
            conn = DBConnection.getConnection();
            String sql = "SELECT question, option_a, option_b, option_c, option_d, correct_option FROM quizzes WHERE title = ?";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, quizTitle);
            rs = stmt.executeQuery();
            ...
            while (rs.next()) {
                ...
                questions.add("A: " + optionA);
                questions.add("B: " + optionB);
                questions.add("C: " + optionC);
                questions.add("D: " + optionD);
                questions.add("Correct: " + correctOption);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
