...
public class DataManager {
    ...
    public List<Quiz> getQuizzesObj() {
        ...
        try {
            conn = DBConnection.getConnection();
            // Updated SQL to select document_name along with id, title and due_date
            String sql = "SELECT id, title, document_name, due_date FROM quizzes";
            stmt = conn.prepareStatement(sql);
            rs = stmt.executeQuery();
            while (rs.next()) {
                // Create Quiz using proper title and document_name
                Quiz quiz = new Quiz(rs.getString("title"), rs.getString("document_name"),
                        rs.getDate("due_date").toLocalDate());
                // Set the quiz id separately
    ...
    // New method: Get quiz submissions for teacher dashboard.
    // Returns a list of display strings showing quiz title, student id, score and
    // attempt date.
    public List<String> getQuizSubmissions() {
        ...
        try {
            conn = DBConnection.getConnection();
            String sql = "SELECT qa.id, q.title, qa.student_id, qa.score, qa.attempt_date " +
            ...
            while (rs.next()) {
                ...
                String display = "Submission ID: " + submissionId + " | Quiz: " + quizTitle +
                        " | Student ID: " + studentId + " | Score: " + score +
                        " | Date: " + attemptDate;
                submissions.add(display);
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null,
                    "Database error when fetching quiz submissions: " + e.getMessage(),
                    "Error", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        } finally {
            try {
    ...
    // New method: Get assignment submissions for teacher dashboard.
    // Returns a list of display strings showing assignment title, student id,
    // submission date and file path.
    public List<String> getAssignmentSubmissions() {
        List<String> submissions = new ArrayList<>();
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
            conn = DBConnection.getConnection();
            String sql = "SELECT s.id, a.title, s.student_id, s.submission_date, s.file_path " +
                    "FROM submissions s " +
            ...
            while (rs.next()) {
                ...
                String display = "Submission ID: " + submissionId + " | Assignment: " + assignmentTitle +
                        " | Student ID: " + studentId + " | Date: " + submissionDate +
                        " | File: " + filePath;
                submissions.add(display);
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null,
                    "Database error when fetching assignment submissions: " + e.getMessage(),
                    "Error", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        } finally {
            try {
